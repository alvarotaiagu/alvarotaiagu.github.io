...
Un pueblo, 39 webs. El portfolio es una calle de noche con los escaparates
...
```
npm install
npm run extract     # visita las 39 webs y saca sus datos → scripts/extracted.json
npm run projects    # seed + extracted + overrides → data/projects.json + JSON-LD + sitemap
npm run shots       # regenera las 78 capturas (1440×900 y 390×844) en webp < 150 KB
npm run og          # compone assets/og.jpg a partir de la propia calle
npm run verify      # Playwright: 39 URLs, capturas, iframes, filtros, foco, longtasks, 400 px
...
halo del escaparate. Ese color se usa tal cual en 34 de los 39; los otros cinco
...
*layout*, no JavaScript: 39 escaparates con rejilla, consultas de contenedor e
...
`prefers-reduced-motion` la marquesina se queda quieta pero los 39 rótulos
